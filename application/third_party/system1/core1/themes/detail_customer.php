...
<br>
<form action="<?php echo base_url();?>seller/profile_filter/<?php echo $cus_id;?>" method="post">
...
<span style="font-size:35px;color:white"><h2 style="color:white;"><?php echo $detail_customer[0]['customer_count'];?></h2><b style="font-size: 26px;">Total Orders</b></span>
</div>
...
<div class="md-card-content uk-flex uk-flex-center uk-flex-middle">
<span style="font-size:35px;color:white;"><h2 style="color:white;"><?php echo $detail_customer[0]['customer_paid'];?></h2><b style="font-size: 26px;">Paid</b></span>
</div>
<div class="md-card-overlay-content">
<div class="uk-clearfix md-card-overlay-header">
</div>
...
<div class="md-card-content uk-flex uk-flex-center uk-flex-middle">
<span style="font-size:35px;color:white"><h2 style="color:white;"><?php echo $detail_customer[0]['customer_pending'];?></h2><b style="font-size: 26px;">Pending</b></span>
</div>
<div class="md-card-overlay-content">
<div class="uk-clearfix md-card-overlay-header">
</div>
...
<div class="md-card-content uk-flex uk-flex-center uk-flex-middle">
<span style="font-size:35px;color:white"><h2 style="color:white;"><?php echo $detail_customer[0]['customer_due'];?></h2><b style="font-size: 26px;">Due</b></span>
</div>
<div class="md-card-overlay-content">
<div class="uk-clearfix md-card-overlay-header">
</div>
...
foreach($b as $c)
{$id=$c['id'];
$query =$this->db->query("SELECT date_format( on_date, '%m' ) as month, SUM( Total_amount) as total_sales FROM invoice WHERE seller_id = '$user_id' and `on_date` LIKE '%-0$id-%' 
...
$pending =$this->db->query("SELECT date_format(on_date, '%m' ) as month, SUM(Total_amount) as total_pending FROM invoice WHERE status='pending' and seller_id = '$user_id' and `on_date` LIKE '%-0$id-%' and `on_date` LIKE '%$month_year-%' and customer_id='$cus_id' and `on_date` LIKE '%2016-%' AND on_date >= date_sub( now( ) , INTERVAL 12 MONTH ) GROUP BY date_format( on_date, '%b' ) ORDER BY date_format( on_date, '%m' ) ASC");
$pending_amount=$pending->result_array();


$paid =$this->db->query("SELECT date_format( add_on, '%m' ) as month, SUM(invoice_paid_amount) as total_paid FROM payment WHERE seller_id = '$user_id' and `add_on` LIKE '%-0$id-%'  and `add_on` LIKE '%$month_year-%' and customer_id='$cus_id'  AND add_on >= date_sub( now( ) , INTERVAL 12 MONTH ) GROUP BY date_format( add_on, '%b' ) ORDER BY date_format( add_on, '%m' ) ASC");
$paid_amount=$paid->result_array();

$due_amount = $amount[0]['total_sales']-$paid_amount[0]['total_paid'];

$canceled =$this->db->query("SELECT date_format(on_date, '%m' ) as month, SUM(Total_amount) as total_canceled FROM invoice WHERE status='canceled' and seller_id = '$user_id' and `on_date` and customer_id='$cus_id' LIKE '%-0$id-%' and `on_date` LIKE '%$month_year-%' and customer_id='$cus_id' AND on_date >= date_sub( now( ) , INTERVAL 12 MONTH ) GROUP BY date_format( on_date, '%b' ) ORDER BY date_format( on_date, '%m' ) ASC");
