...
$config = array();
			...
			//	$message['html'] = file_get_contents("http://dummyimage.com/"); 
				$message['html'] = '<body>Dear User,<br /><br />Please click on the below activation link to verify your email address.<br /><br />'.base_url().'user_ctrl/verify/' . md5($this->input->post('email')) . '<br /><br /><br />Thanks<br />FinacBooks Team</body>';
				$ch = curl_init(); 
...
public function recover_password(){
    $this->form_validation->set_rules('email_reset', 'Email', 'required|trim|xss_clean|callback_validate_credentials');
        ...
        if($this->user_model->email_exists()){
            ...
            $this->email->from('support@morelifelondon.com', "Site");
            $this->email->to($this->input->post('email_reset'));
            $this->email->subject("Reset your Password");

            $message = "<p>This email has been sent as a request to reset our password</p>";
            $message .= "<p><a href='".base_url()."user_ctrl/reset_password/$temp_pass'>Click here </a>if you want to reset your password,
            ...
            if($this->email->send()){
                ...
                if($this->user_model->temp_reset_password($temp_pass)){
				 ...
				 redirect('user_ctrl?email=check your email for instructions, thank you');
                }
            }
            else{
