...
class Ajax extends CI_Controller
{
    ...
    public function get_subcustomer()
    {
        $pid = $_REQUEST['pid'];
        $pid = explode("::", $pid);
        $id = $pid[0];
        $html = "";
        $ee = "select * from customers_contacts where cid='$id'";
        $customers_contacts = $this->db->query($ee)->result_array();
        $html .= "<option value=''>Please select</option>";
        if (!empty($customers_contacts)) {
            foreach ($customers_contacts as $edd) {
                $html .= "<option value='{$edd['name']}'>{$edd['name']} - {$edd['email']}</option>";
            }
        }
        echo $html;
        die;
    ...
    public function get_subpriciple()
    {
        ...
        $html = "";
        $ee = "select * from principal_contacts where pid='$id'";
        $customers_contacts = $this->db->query($ee)->result_array();
        $html .= "<option value=''>Please select</option>";
        if (!empty($customers_contacts)) {
            foreach ($customers_contacts as $edd) {
    ...
    public function getproducts()
    {
		...
		$ee = "SELECT * FROM customers_principal_assigned LEFT JOIN principal_products ON customers_principal_assigned.product_id = principal_products.id WHERE customers_principal_assigned.pid = '$pid' AND principal_products.pid ='$pid' AND customers_principal_assigned.cid='$cid' GROUP BY principal_products.product_name;";
        //New code implemented for Set product name in alphabetic
		//$ee = "select * from customers_principal_assigned INNER JOIN principal_products ON customers_principal_assigned.pid=principal_products.pid where customers_principal_assigned.pid='$pid' and customers_principal_assigned.cid='$cid' group by principal_products.product_name";
        ...
        $pro_assidnedhtml .= "
                 <table class='order-list display table table-bordered table-hover veg_item' id='caltbl' >
                    <thead>
                        <tr>
                            <th width='5%'></th>
        ...
        if (!empty($customer_product)) {
            foreach ($customer_product as $productdata) {
                $productid = $productdata['product_id'];
                //$pname = "SELECT * FROM `principal_products` WHERE `id`= '$productid' order by product_name";
                ...
                $pro_assidnedhtml .= "<tr>
                        <td style=' text-align:center;vertical-align: middle;' >
                            ...
                            <td><input type='text' name='price[{$product_name['id']}]' id='price{$product_name['id']}' class='form-control changesNo' onkeyup='calc({$product_name['id']})' onchange='calc({$product_name['id']})'  style='width: 100%;' value='{$productdata['price']}'></td>
                           ...
                           <td> <input type='number' min='1' name='cunt[{$product_name['id']}]' id='cunt$product_name[id]' class='form-control changesNo'  onkeyup='calc({$product_name['id']})' value='1' onchange='calc({$product_name['id']})' style='width: 100%;' value=''>
        ...
        if (isset($_REQUEST['pid'])) {
            ...
            foreach ($all as $pid) {
                ...
                if (!empty($principal_contacts)) {
                    $principal_contactshtml .= "<select class='js-select2-template uk-width-1-1' id='principal_contacts' name='principal_contacts' style='height: 28px;    border-radius: 4px;' required><option>Select Principal Contacts</option>";
...
//                        $principal_contactshtml.="<div class='checkbox'><label><input type='hidden' class='principal_contacts_email principal_contacts_email{$contacts['id']} ' /><input type='checkbox' name='principal_contacts[]' class='principal_contacts principal_contacts{$contacts['id']}' value='{$contacts['id']}'>{$contacts['name']} </label></div>";
                ...
                if (!empty($customers_principal_assigned)) {
                    $customers_principal_assigned_html .= "<select class='js-select2-template uk-width-1-1' onchange='showproducts()' id='customers_principal_assigned' name='customers_principal_assigned' style='height: 28px;border-radius: 4px;' required>";
                    //$customers_principal_assigned_html .= "<option>Select Assign Customers</option>";
...
//                        $customers_principal_assigned_html.="<div class='checkbox'><label><input type='checkbox' onclick='get_products({$customerdata['customer_person']})' name='principal_contacts' class='customers_principal_assigned customers_principal_assigned{$c_p_a['cid']}' value='{$c_p_a['id']}'>{$customerdata['customer_person']} </label></div>";
                ...
                $customers_principal_sname_html .= "<span>Principal MSG ID : {$principal_short['short_name']}</span>";
