     ...
     public function orderview($id)
    {
        if ($this->session->userdata('logged_in')) {
			...
			if(!empty($session_data)){
				$data['list_order'] = $this->product_model->list_order_model_new($session_data['user_id']);
			}
            
            $this->load->view('admin_seller/view_order', $data);
        } else {
            redirect('user_ctrl/login', 'refresh');
        }
    }
...
//echo'<script>window.location.href = "'.site_url().'/'.$url.'";</script>';
    ...
    public function saveorder()
    {
	  ...
	  $pr_id = $this->input->post('principal_id');
	$principal_no =0;	
			$querypr = $this->db->query("SELECT principal_msg_id FROM `orders` WHERE `pid`='$pr_id' order BY id DESC LIMIT 0,1");
            $datapr = $querypr->result_array();
			if(!empty($datapr)){
			$principal_no =$datapr[0]['principal_msg_id']; 
			}
            extract($data);
            $data = array(
                'pid' => $this->input->post('principal_id'),
                ...
                'delivery_terms' => $this->input->post('delivery_terms'),
                'seller_id' => $user_id,
				'principal_msg_id' => $principal_no+1
            );
...
//echo'<script>window.location.href = "'.site_url().'/'.$url.'";</script>';
    ...
    public function savebusinessupdate()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            if ($this->product_model->savebusinessupdate($data, $_REQUEST, $_FILES) == TRUE) {
...
//echo'<script>window.location.href = "'.site_url().'/'.$url.'";</script>';
    ...
    public function listemails()
    {
        if ($this->session->userdata('logged_in')) {
            ...
            $data['getorderemailidsdata'] = $this->product_model->getorderemailidsdatanew($data['user_id']);
    ...
    public function delete_businesss($id)
    {
        if ($this->session->userdata('logged_in')) {
            $session_data = $this->session->userdata('logged_in');

            $delete = $this->product_model->delete_business_model($id);
            if ($delete == TRUE) {
                // redirect('manageCustomers?customer=deleted');
                $url = 'seller/listemails?customer=deleted';
                echo '<script>window.location.href = "' . site_url() . '/' . $url . '";</script>';
            }
        } else {
            redirect('user_ctrl/login', 'refresh');
    ...
    public function sendemail_businesss($id)
    {
        if ($this->session->userdata('logged_in')) {
            $session_data = $this->session->userdata('logged_in');
            $data['user_id'] = $session_data['user_id'];
            ...
            $data['records4'] = $this->profile_model->company_profile($data['user_id']);
            $id = $_REQUEST['id'];
            $order_dd = $this->db->query("select * from business where id='{$id}' and del='0' order by id desc")->row_array();
            if (isset($order_dd) && !empty($order_dd)) {


                $cid = $order_dd['cid'];
                $messages = $order_dd['messages'];
                $messages = $order_dd['messages'];
                $messageid = $order_dd['messageid'];

                /*$separator = md5(time());
                $adminmeail = "risus@support.com";
                ///////////HEADERS INFORMATION////////////
                // main header (multipart mandatory) message
                $headers = "From: Risus. <" . $adminmeail . ">" . $eol;
                ...
                $piddata = $order_dd['pid'];
                $pidcid = $order_dd['pidcid'];
                $pid = $pcname = 0;
                $ddddd = explode('::', $piddata);
                if ($ddddd[0]) {
                    $pid = $ddddd[0];
                }
                if ($ddddd[1]) {
                ...
                if (!empty($pid) && !empty($pidcid)) {
                    $dddd = "select * from principal_contacts where pid='{$pid}' and name='$pidcid' limit 1";
                    $pppdata = $this->db->query($dddd)->row_array();
                    if (isset($pppdata['email']) && !empty($pppdata['email'])) {
                        $email = $pppdata['email'];
                        $email1 = "sg@risusventures.com";
                    }
                ...
                $cidata = $order_dd['cid'];
                $cidcid = $order_dd['cidcid'];
                $cid = $ccname = 0;
                $ddddd = explode('::', $cidata);
                ...
                if ($ddddd[1]) {
                    $ccname = $ddddd[1];
                    $ccname = addslashes($ccname);
                }
                if (!empty($cid) && !empty($cidcid)) {
                    $dddd = "select * from customers_contacts where cid='{$cid}' and name='$cidcid' limit 1";
                    $pppdata = $this->db->query($dddd)->row_array();
                    if (isset($pppdata['email']) && !empty($pppdata['email'])) {
                        $email2 = $pppdata['email'];
                    }
                }
                ...
                try {
                    ...
                    $this->phpmailer->Username = 'sg@risusventures.com';
                    $this->phpmailer->Password = 'Protect#1';*/
                    $this->phpmailer->setFrom('sg@risusventures.com', 'Risus Ventures');
                    $this->phpmailer->addAddress($email2);
                    $this->phpmailer->addBCC($email1);
                    $filename = $order_dd['filename'];
                    echo $filename;
                    if (!empty($filename)) {
                        $this->phpmailer->addAttachment('/var/www/vhosts/risusventures.com/httpdocs/usermanagement2/images/' . $filename);
                    }
                    $this->phpmailer->isHTML(true);
                    $this->phpmailer->Subject = 'Risus Ventures - ' . $messageid;
                    $this->phpmailer->Body = $messages;
                    $this->phpmailer->send();
                } catch (\Exception $e) {
                    echo 'Message could not be sent. Mailer Error: ', $this->phpmailer->ErrorInfo;
                }

                $url = 'seller/listemails?customer=email';
                echo '<script>window.location.href = "' . site_url() . '/' . $url . '";</script>';
                die;
    ...
    public function update_businesss()
    {
		$id = $this->input->get('id');
        if ($this->session->userdata('logged_in')) {
            $session_data = $this->session->userdata('logged_in');
            ...
            $data['user_email'] = $session_data['user_email'];
            $data['records4'] = $this->profile_model->company_profile($data['user_id']);
            $data['getorderemailidsdata'] = $this->product_model->getorderemailidsdata($id);
			$data['data'] = $data;
            $this->load->view('admin_seller/update_businesss', $data);
	...
	public function updateorder1()
    {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $data['list_principal'] = $this->product_model->list_principle_model($data['user_id']);
            // echo '<pre>'; print_r($data); echo '</pre>';
            $this->load->view('admin_seller/test_order', $data);
	...
	public function AllOrderDetail($request='')
    {
        if ($this->session->userdata('logged_in')) {
            $session_data = $this->session->userdata('logged_in');
            $data['user_email'] = $session_data['user_email'];
            ...
            $data['records4'] = $this->profile_model->company_profile($data['user_id']);
            $data['list_order'] = $this->product_model->list_order_model_new($data['user_id']);
            $this->load->view('admin_seller/backuppage', $data);
            // echo '<pre>';print_r($data['list_order']);echo'</pre>';
	...
	function downloadCheck(){
		...
		//exit();
		redirect('seller/BackupList', 'refresh');
	}
	
	function BackupList($data=''){
		$this->load->view('admin_seller/backuplist', $data);
	}
	...
	$path='/var/www/vhosts/risusventures.com/httpdocs/usermanagement2/';
	$this->zip->read_dir($path, TRUE);
	$date = date('Y-m-d H:i:s');
	$this->zip->archive('/var/www/vhosts/risusventures.com/httpdocs/backup/files/project_'.$date.'.zip');
	//$this->zip->archive(FCPATH.'/archives/'.$filename);
	...
	$fileName1 = 'db'.date('Y-m-d H:i:s').'.zip';
    // Load the file helper and write the file to your server
    $this->load->helper('file');
    write_file('/var/www/vhosts/risusventures.com/httpdocs/backup/db/'.$fileName1, $backup);

    // Load the download helper and send the file to your desktop
    // $this->load->helper('download');
    // force_download($fileName, $backup);
	//redirect('seller/project_backup', 'refresh');
}
...
//     echo '<pre>';print_r($start_date);echo'</pre>';
    ...
    public function export_report()
    {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            $data['list_customer'] = $this->product_model->list_customer_model_filter($data['user_id'], $start_date, $end_date);
            $this->load->view('admin_seller/export_report', $data);
        } else {
            redirect('user_ctrl/login', 'refresh');
        }
    }
	
	public function export_report_back()
    {
		$track_id = $_GET['code'];
        if ($this->session->userdata('logged_in')) {
            $session_data = $this->session->userdata('logged_in');
    ...
    function sales_report()
    {
        ...
        if ($this->session->userdata('logged_in')) {
            ...
            if ($start_date && $end_date) {
                ...
                foreach ($c as $invoice_paid) {
                    $paid = $invoice_paid['paid_amount'];
                }
...
public function sendOrdermail()
 ...
 if ($this->session->userdata('logged_in')) {
     $img = $this->input->post('maildata',false);
	 $to_email = $this->input->post('to_email',false);
	 $id = $this->input->post('oid',false);
...
$this->email->initialize($config);
	  ...
	  $this->email->set_header('MIME-Version', '1.0; charset=utf-8');
        ...
        $this->email->from('info@risusventures.com','Risusventures.com');
        //$this->email->to($session_data['user_email']);
	$this->email->to($to_email);
        $this->email->subject('Your Order Details');
	 //$img = $this->load->view('emailFormate.php',$img,TRUE);
        $this->email->message($img);

        if ($this->email->send()) {
			$this->session->set_flashdata('message_name', 'Your Email has successfully been sent');
            //echo 'Your Email has successfully been sent.';
        } else {
            show_error($this->email->print_debugger());
        }
        redirect('orderview/'.$id, 'refresh');
...
public function reportsfilter()
	...
	$cid =  $_GET['cid'];
	$que = $_GET['duration'];
	if($que=='q1'){ $duration = array(1,2,3);
	}elseif($que=='q2'){ 
	$duration = array(4,5,6); 
	}elseif($que=='q3'){ 
	$duration = array(7,8,9); 
	}else{
		$que = array(10,11,12); }
...
LEFT JOIN orders_products ON orders_products.order_id = orders.id where customers.customer_id='$cid'
GROUP BY
    orders.id
ORDER BY
    orders.id ASC";
...
if (!empty($orders)) {  
echo '<table class="table" id="order1" style="border: 2px solid whitesmoke !important;">
                                        <thead>
                                        <tr>
											...
											<th><strong>Invoice Amount ( USD / EUR )</strong></th>
		...
		$csum = 0;$dsum = 0;$esum = 0;
                                            foreach ($orders as $single_order) {
												...
												//echo $single_order['order_no'] . " / " . $orderdate;
                                                if (!empty($orderProducts)) {
                                                    ...
                                                    foreach ($orderProducts as $sp) {
														...
														if(in_array($month,$duration)){
														$csum+= (($p_commision * $p_subtotal) / 100);
															$dsum+= $single_order['totalusd'];
															$esum+= $single_order['totaleur'];
															$inamt = 0;
															if($single_order['totalusd']!= 0.0000){
																$inamt = $single_order['totalusd'].' USD';
															}
															if($single_order['totaleur']!= 0.0000){
																$inamt = $single_order['totaleur'].' EUR';
															}
echo '<tr><td>'.$p_name.'</td><td>'.$single_order['order_no'] . " / " . $orderdate.'</td><td>'.$p_quntity.'</td><td>'.(($p_commision * $p_subtotal) / 100).'</td><td>'.$inamt.'</td></tr>';	
														}
	 ...
	 echo '<tr style="background: #615959;color: white;"><td><b>Total</b></td><td></td><td></td><td>'.$csum.'</td><td>'.$dsum.'USD
	/ '.$esum.'EUR</td></tr>';
	 echo '</tbody></table>';
}
