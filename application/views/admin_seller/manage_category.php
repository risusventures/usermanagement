                    ...
                    <div id="printablediv" style="width: 100%;height:auto;">
                        <table id="dt_tableTools" id="printablediv" class="uk-table" cellspacing="0" width="100%">
                            ...
                            <?php foreach ($records as $row) {
                                $ci_id = $row->ci_id; ?>
                                <tr>
                                    <td><a href="<?php echo base_url() ?>productsCategory/<?php echo $row->ci_id; ?>"
                                           style="font-size:15px;text-transform: capitalize;"><?php echo $row->product_category; ?></a>
                                    </td>
                                    <td><span class="uk-badge uk-badge-muted"
                                              style="background:none;color:black;font-size:15px;">
                    <?php
                    $subquery = $this->db->query("select  *  from add_products where product_group='$ci_id' ");
                    $total = $subquery->num_rows();
                    echo $total; ?>
                    </span></td>
                                    <td><img src="<?php echo base_url(); ?>upload/<?php if ($row->category_image) {
                                            echo $row->category_image;
                                        } else {
                                            echo 'cat.jpg';
                                        } ?>" style="height:70px;width:70px;"></td>
                                    <td><span class="switchery switchery-default"
                                              style="border-color: rgba(0, 150, 136, 0.498039); box-shadow: rgba(0, 150, 136, 0.498039) 0px 0px 0px 7px inset; transition: border 0.4s, box-shadow 0.4s, background-color 1.2s; background-color: rgba(0, 150, 136, 0.498039);"><small
                                                    style="left: 18px; transition: background-color 0.4s, left 0.2s; background-color: rgb(0, 150, 136);"></small></span>
                                    </td>
                                    <td><a href="<?php echo base_url() ?>productsCategory/<?php echo $row->ci_id; ?>"
                                           data-uk-tooltip="{pos:'bottom'}" title="View All Products"><i
                                                    class="material-icons md-24">&#xE8F4;</i></a>
                                        <a href="<?php echo base_url() ?>editCategory/<?php echo $row->ci_id; ?>"
                                           data-uk-tooltip="{pos:'bottom'}" title="Edit Category"><i
                                                    class="material-icons">create</i></a>
                                        <a href="<?php echo base_url() ?>seller/delete_category_id/<?php echo $row->ci_id; ?>"
                                           onclick="return ConfirmDialog()" data-uk-tooltip="{pos:'bottom'}"
                                           title="delete"><i class="material-icons md-24">&#xE872;</i></a>
                                        <a href="<?php echo base_url() ?>addproductsCategory/<?php echo $row->ci_id; ?>"
                                           data-uk-tooltip="{pos:'bottom'}" title="Add Product"><i
                                                    class="material-icons md-24">note_add</i></a>
                                    </td>
                                </tr>
                            <?php } ?>
                            </tbody>
